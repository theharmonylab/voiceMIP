...
if (TRUE){
  # data cleaning
  # count the number of filtering from too_small
  toosmall <- read.csv('demo_talk/to_wavs/online_small_files_log.csv')
  # align to noVoice_MIP_online_FINAL_clean.csv
  df_online <- read.csv('demo_talk/dat/noVoice_MIP_online_FINAL_clean.csv')
  df_online <- dplyr::filter(df_online, pPlace != 'mollan')
  df <-  df_online %>%
    inner_join(toosmall, by = "pRandID")
  table(df$tooSmall)
  # 0   1 - too small 
  # 714  25 
}
